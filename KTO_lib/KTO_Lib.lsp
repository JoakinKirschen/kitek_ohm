;-------------------------------------------------------------------------------
; Program Name: KTO_Lib.lsp [Block Library R17]
; Created By:   Terry Miller (Email: terrycadd@yahoo.com)
;               (URL: http://web2.airmail.net/terrycad)
; Date Created: 1-1-00
; Function:     Loads Block Library functions to insert blocks and adds blocks
;               to Block Libraries.
; Note:         KTO_Lib requires functions inside of GetIcon.lsp.
;-------------------------------------------------------------------------------
; Quick Block Library Installation Steps:
; 1. Copy KTO_Lib.lsp and KTO_Lib.dcl to a folder in your AutoCAD's search path.
; 2. Start AutoCAD and at the Command line type (load "KTO_Lib.lsp") to load
;    Block Library.
; 3. Then on the command line type (setq *KTO_Lib* t) so as to bypass the Block
...
; 3    TM   3-1-00    Revised c:InBlocks and c:All_KTO_Lib functions.
...
; 7    TM   1-1-01    Included KTO_Lib as the main command function and added
...
;                     into a drawing.
;-------------------------------------------------------------------------------
;   Note: Block Library saves a backup of your drawing as C:\Custom\KTO_Lib.dwg
;   before it creates slides and wblocks.  If the program is interrupted or
;   canceled open C:\Custom\KTO_Lib.dwg as a backup.
;-------------------------------------------------------------------------------
; Overview of Main Functions
;-------------------------------------------------------------------------------
; c:ALLBL and c:All_KTO_Lib [All Blocks to Library] - Select Library to add all
;   blocks in drawing.
;
; All_KTO_Lib [All Blocks to Library] - Adds all blocks in drawing to Library.
;   Example: (All_KTO_Lib "Notes" (strcat BlockLibBase$ "\\Custom\\Notes\\Notes.def" nil)
;   Returns: Creates or appends a Notes Library DefFile definition file in the
;   C:\Custom\Notes folder for displaying slides and inserts blocks from the
;   same folder by default.
;
; c:BLIB - Call the function KTO_Lib with default arguments.
;
; c:KTO_Lib [Block Library] - Main command function that calls c:Sel_Lib
;  [Select Block Library]
;
; KTO_Lib [Block Library] - Inserts blocks from Library or calls the function
;   KTO_Mgr.
;   Example: (KTO_Lib "Electronic Symbols" (strcat BlockLibBase$ "\\Custom\\Elec_Sym\\Elec_Sym.def"
;   (strcat BlockLibBase$ "\\Custom\\Elec_Blk\\Elec_Blk.def")
;   Returns: Creates or reads an Electronics Symbols Library DefFile definition
;   file in the C:\Custom\Elec_Sym folder for displaying slides and inserts
;   blocks from the C:\Custom\Elec_Blk folder.
;
; c:BLM and c:KTO_Mgr - Call the function KTO_Mgr with default arguments.
;
; KTO_Mgr [Block Library Manager] - Adds blocks to the Library or calls the
;   function Edit_Lib if no blocks found in drawing to add.
;   Example: (KTO_Mgr "Notes" (strcat BlockLibBase$ "\\Custom\\Notes\\Notes.def"
;   (strcat BlockLibBase$ "\\Custom\\Notes\\Notes.def")
;   Returns: Adds a block to the C:\Custom\Notes folder or calls the function
;   Edit_Lib if no blocks found in drawing to add.
;
; c:BLMENU and c:KTO_Menu [Block Library Menu] - Select a Block Library Command
...
;   data in the DefFile.
;   Example: (Edit_Lib "Notes" (strcat BlockLibBase$ "\\Custom\\Notes\\Notes.def")
;   Returns: Rearrange or delete lines of block insertion data for the Notes
;   Library in the C:\Custom\Notes\Notes.def DefFile.
;
...
;   Example: (User_Lib "Weld Symbols" (strcat BlockLibBase$ "\\Custom\\Weld_Sym\\Weld_Sym.def" nil)
;   Returns: Reads a Weld Symbols Library DefFile definition file in the
;   C:\Custom\Weld_Sym folder for displaying slides and inserts blocks from
...
;-------------------------------------------------------------------------------
; c:ALLBL - Shortcut for c:All_KTO_Lib. [All Blocks to Library]
;-------------------------------------------------------------------------------
(defun c:ALLBL () (c:All_KTO_Lib))
;-------------------------------------------------------------------------------
; c:BLIB - Shortcut that calls function KTO_Lib with default arguments.
;-------------------------------------------------------------------------------
(defun c:BLIB () (KTO_Lib nil nil nil))
;-------------------------------------------------------------------------------
; c:KTO_LIB - Shortcut for c:Sel_Lib. [Select Block Library]
;-------------------------------------------------------------------------------
(defun c:KTO_LIB () (c:Sel_Lib))
;-------------------------------------------------------------------------------
; c:BLM - Shortcut for c:KTO_Mgr. [Block Library Manager]
;-------------------------------------------------------------------------------
(defun c:BLM () (KTO_Mgr nil nil nil))
;-------------------------------------------------------------------------------
; c:BLMENU - Shortcut for c:KTO_Menu. [Block Library Menu]
;-------------------------------------------------------------------------------
(defun c:BLMENU () (c:KTO_Menu) (princ))
;-------------------------------------------------------------------------------
...
;-------------------------------------------------------------------------------
; c:All_KTO_Lib [All Blocks to Library] - Select Library to add all blocks in
; drawing.
;-------------------------------------------------------------------------------
(if (findfile "KTO_Lib.lsp")(setq BlockLibBase$ (vl-filename-directory (findfile "KTO_Lib.lsp")))(exit))

(defun c:All_KTO_Lib (/ BlockLib@ BlkList@ Option# Cnt# DatFile$ Dcl_Id% EOF
  ...
  ;-----------------------------------------------------------------------------
  ; c:All_KTO_Lib subfunctions = LibrarySub:
  ;-----------------------------------------------------------------------------
  ; LibrarySub:
  ;-----------------------------------------------------------------------------
  (defun LibrarySub: ()
    (setq nthLibrary$ (get_tile "library")
          ...
          *KTO_Lib* (nth intLib# TitleLib@)
    );setq
    (set_tile "library" nthLibrary$)
  );defun LibrarySub:
  ;-----------------------------------------------------------------------------
  ; c:All_KTO_Lib - Start Main Function
  ;-----------------------------------------------------------------------------
  (setq LibTitle$ "All Blocks to")
  (setq PathDat$ (strcat BlockLibBase$ "\\Custom\\KTO_Lib\\Sel_Lib.dat"))
    ...
    (if Text$
      (if (= (substr Text$ 1 1) Q$)
        (progn
            ...
            (if (= Mid$ "\",\"")
              (progn
                ...
                (cond
                  ...
                  ((= Field# 2)
                    (if SlideLib@
                      (setq SlideLib@ (append SlideLib@ (list (strcat BlockLibBase$ Item$))))
                      (setq SlideLib@ (list (strcat BlockLibBase$ Item$)))
                    );if
                  );Field# 2
                  (t (exit))
                );cond
          ...
          (if BlockLib@
            (setq BlockLib@ (append BlockLib@ (list (strcat BlockLibBase$ Item$))))
            (setq BlockLib@ (list (strcat BlockLibBase$ Item$)))
          );if
        );progn
      );if
      (setq EOF t)
  ...
  (close FileName%)
  ;-----------------------------------------------------------------------------
  ; Load DCL dialog: Library in KTO_Lib.dcl
  ;-----------------------------------------------------------------------------
  (if (and (= (type *KTO_Lib*) 'STR) (member *KTO_Lib* TitleLib@))
    (setq intLib# (- (length TitleLib@) (length (member *KTO_Lib* TitleLib@)))
  ...
  (setq Dcl_Id% (load_dialog "KTO_Lib.dcl"))
  (while (/= Option# 1)
...
;-------------------------------------------------------------------------------
; All_KTO_Lib {All Blocks to Library] - Add all blocks in drawing to Library.
; Arguments: 3
...
;-------------------------------------------------------------------------------
(defun All_KTO_Lib (LibTitle$ PathDef$ PathBlk$ / Attribs$ Trim$ Allign$ BlkList@ BlkPath$
  ...
  (if (null PathDef$)
    (setq PathDef$ (strcat BlockLibBase$ "\\Custom\\KTO_Lib\\KTO_Lib.def"))
  );if
...
;-------------------------------------------------------------------------------
; KTO_Lib [Block Library] - Inserts blocks from Library.
; Arguments: 3
;   LibTitle$ = Library title
;   PathDef$ = Pathname and DefFile name for slides
;   PathBlk$ = Pathname and DefFile name for blocks
; Returns: Inserts a block or calls function KTO_Mgr.
;-------------------------------------------------------------------------------
(defun KTO_Lib (LibTitle$ PathDef$ PathBlk$ / Attribs$ AttribsList@ BlkList@
    BlkPath$ Block$ Trim$ Allign$ BlockLen# Option# Cnt# DefFile$ DefPath$ Dcl_Id% EOF
    Explode$ ExplodeList@ TrimList@ AllignList@ Field# FileName% ImageName$ Item$ Layer$ LayerList@
    ...
    (if Text$
      (if (= (substr Text$ 1 1) Q$)
        (progn
          ...
          (setq Cnt# 1)
          (setq Field# 1)
          (while (and (<= Cnt# (strlen Text$)) (< Field# 8))
            (setq Mid$ (substr Text$ Cnt# 3))
            (if (= Mid$ "\",\"")
              (progn
                (setq Item$
...
>>>>>>> origin/master
          ...
          (if AllignList@
            (setq AllignList@ (append AllignList@ (list Item$)))
            (setq AllignList@ (list Item$))
          );if
        );progn
      );if
      (setq EOF t)
  ...
  ;-----------------------------------------------------------------------------
  ; Load DCL dialog: KTO_Lib in KTO_Lib.dcl
  ;-----------------------------------------------------------------------------
  (princ "\nSelect a block or option: ")(princ)
  (setq Dcl_Id% (load_dialog "KTO_Lib.dcl"))
  (while (/= Option# 1)
    (new_dialog "KTO_Lib" Dcl_Id%)
  ...
  (if (and SlideRef$ Pick)
    (progn
      ...
      (setq Block$   (nth Ref# BlkList@)
            ...
            Trim$    (nth Ref# TrimList@)
            Allign$  (nth Ref# AllignList@)
      );setq
      (setq PathBlock$ (strcat BlkPath$ Block$ ".dwg"))
...
>>>>>>> origin/master
  ...
  (if (= Manager$ "Yes")
    (KTO_Mgr LibTitle$ PathDef$ PathBlk$)
  );if
  (setq *error* Old_error)
  (princ)
);defun KTO_Lib
;-------------------------------------------------------------------------------
; KTO_Lib_Msg [Block Library Message] - Installation Instructions
;-------------------------------------------------------------------------------
(defun KTO_Lib_Msg (/ Dcl_Id%)
  ;-----------------------------------------------------------------------------
  ; Load DCL dialog: KTO_Lib_Msg in KTO_Lib.dcl
  ;-----------------------------------------------------------------------------
  (setq Dcl_Id% (load_dialog "KTO_Lib.dcl"))
  (new_dialog "KTO_Lib_Msg" Dcl_Id%)
  (set_tile "msg1"
    "Note: Add the following line to the AutoLISP file that will be loading \"KTO_Lib.lsp\".")
  (set_tile "msg2"
    "(load \"KTO_Lib.lsp\")  (setq *KTO_Lib* t)")
  (set_tile "msg3"
    "Block Library saves a backup of your drawing as BlockLibDir \BlockLib\\Custom\\KTO_Lib.dwg before")
  (set_tile "msg4"
    "it creates slides and wblocks.  If the program is interrupted or canceled, you may")
  (set_tile "msg5"
    "open BlockLibBase$ \BlockLib\\Custom\\KTO_Lib.dwg as a backup.")
...
;-------------------------------------------------------------------------------
; c:KTO_Mgr - Calls function KTO_Mgr with default arguments.
;-------------------------------------------------------------------------------
(defun c:KTO_Mgr () (KTO_Mgr nil nil nil) (princ))
;-------------------------------------------------------------------------------
; KTO_Mgr [Block Library Manager] - Manages blocks for Library.
; Arguments: 3
;   LibTitle$ = Library title
;   PathDef$ = Pathname and DefFile name for slides
;   PathBlk$ = Pathname and DefFile name for blocks
...
; in drawing to add.
;-------------------------------------------------------------------------------
(defun KTO_Mgr (LibTitle$ PathDef$ PathBlk$ / Attribs$ Trim$ Allign$ BlkList@ BlkPath$
    KTO_Lib_def: Block$ BlockMgr: DefFile$ DefPath$ Dot$ EditLib$ Explode$
  ...
  (Custom_Dirs LibTitle$ PathDef$ PathBlk$)
  ;-----------------------------------------------------------------------------
  ; KTO_Mgr subfunctions = BlockMgr:, Mslide_Wblock:, KTO_Lib_def:
  ;-----------------------------------------------------------------------------
  ; BlockMgr: - Controls dialog KTO_Mgr to Manage blocks for Library.
  ;-----------------------------------------------------------------------------
  (defun BlockMgr: (/ AddLibSub: Attribs$ BlkList@ Block$ BlockSub: Option# Cnt#
    ...
    (defun BlockSub: ()
      ...
      (slide_image 0 0 (dimx_tile "blocksld") (dimy_tile "blocksld")
        (strcat BlockLibBase$ "\\Custom\\KTO_Sld\\" Block$ ".sld")
      );slide_image
      (end_image)
      (if (= (cdr (assoc 70 (tblsearch "BLOCK" Block$))) 2)
        (setq Attribs$ "1")
    ...
    (defun AddLibSub: ()
      (setq MslideData@
       '(Block$ Layer$ Point$ Scale$ Explode$ Attribs$ Trim$ Allign$ Dot$)
      );setq
    ...
    (defun View_Blks: (BlkList@ / BlockLen# Option# Dcl_Id% ImageName$ No_Blks#
      ...
      ;-------------------------------------------------------------------------
      ; Load DCL dialog: Dwg_Blks in KTO_Lib.dcl
      ;-------------------------------------------------------------------------
      (princ "\nView Blocks ")
      (princ "\nSelect a block or option: ")(princ)
      (setq Dcl_Id% (load_dialog "KTO_Lib.dcl"))
      (while (/= Option# 1)
        ...
        (repeat (fix No_Blks#)
          ...
          (slide_image 0 0
            (dimx_tile (strcat "sld" (itoa Rep#)))
            (dimy_tile (strcat "sld" (itoa Rep#)))
            (strcat BlockLibBase$ "\\Custom\\KTO_Sld\\" ImageName$ ".sld")
          );slide_image
          (end_image)
          (set_tile
            (strcat "sld" (itoa Rep#) "text") ImageName$
    ...
    ;---------------------------------------------------------------------------
    (if *KTO_Mgr@
      (if (and (member (nth 0 *KTO_Mgr@) BlkList@)(member (nth 1 *KTO_Mgr@) LayList@))
        (setq Block$ (nth 0 *KTO_Mgr@)
              ...
              Explode$ (nth 4 *KTO_Mgr@)
              Dot$ (nth 5 *KTO_Mgr@)
              Trim$ (nth 6 *KTO_Mgr@)
              Allign$ (nth 7 *KTO_Mgr@)
        );setq
      );if
    );if
    (if (not Block$)
    ...
    ;---------------------------------------------------------------------------
    ; Load DCL dialog: KTO_Mgr in KTO_Lib.dcl
    ;---------------------------------------------------------------------------
    (setq Dcl_Id% (load_dialog "KTO_Lib.dcl"))
    (setq Option# 99)
    (while (/= Option# 1)
      (new_dialog "KTO_Mgr" Dcl_Id%)
      ...
      (action_tile "cancel" "(done_dialog 2)")
      (setq Option# (start_dialog))
      (setq *KTO_Mgr@ (list Block$ Layer$ Point$ Scale$ Explode$ Dot$ "0" "0"))
      (if (= Option# 5);viewblk
        (progn
          (ViewBlkSub:)
          (setq Option# 99)
  ...
  ;-----------------------------------------------------------------------------
  ; KTO_Lib_def: - Append PathDef$ file with block insertion data.
  ; Arguments: 1
  ...
  (if (and MslideList@ BlkList@)
    (progn
      (setq Block$   (nth 0 MslideList@)
            ...
            Dot$     (nth 8 MslideList@)
            Trim$    (nth 6 MslideList@)
            Allign$  (nth 7 MslideList@)
      );setq
      ...
      (setq MslideList@ (list
        ...
        (eval Scale$) (eval Explode$) (eval Attribs$) (eval Trim$) (eval Allign$) )
      );setq
...
;-------------------------------------------------------------------------------
; c:KTO_Menu [Block Library Menu]
;-------------------------------------------------------------------------------
(defun c:KTO_Menu (/ Cmd$ Dcl_Id% GetShortCut: Old_error ShortCutCmd$)
  ;-----------------------------------------------------------------------------
  ; c:KTO_Menu subfunction = GetShortCut:
  ;-----------------------------------------------------------------------------
  ; GetShortCut:
  ;-----------------------------------------------------------------------------
  ...
  ;-----------------------------------------------------------------------------
  ; c:KTO_Menu - Start Main Function
  ;-----------------------------------------------------------------------------
  (setq Old_error *error* *error* *BL_error*)
  ;-----------------------------------------------------------------------------
  ; Load DCL dialog: KTO_Menu in KTO_Lib.dcl
  ;-----------------------------------------------------------------------------
  (princ "\nBlock Library Menu")
  (princ "\nSelect a Block Library Menu Command or option: ")(princ)
  (setq Dcl_Id% (load_dialog "KTO_Lib.dcl"))
  (new_dialog "KTO_Menu" Dcl_Id%)
  (setq CommandList@ (list
    "ALLBL\tAll_KTO_Lib\tAll Blocks to Library"
    "BLM\tKTO_Mgr\tBlock Library Manager"
    "BLMENU\tKTO_Menu\tBlock Library Menu"
...
(defun BlkSlides (BlkList@ / Block$ Color0# NewBlkList@ PathSld$ Pt1@ Pt2@ RealBlk& UcsIcon#
  ...
  (runapp (strcat "DEL " "\"" BlockLibBase$ "\\Custom\\KTO_Sld\\*.sld"  "\"") nil);Delete old temp slides
  (setq Pt1@ (polar (getvar "VIEWCTR") (* pi 0.5)(/ (getvar "VIEWSIZE") 2.0)))
  (setq Pt2@ (polar Pt1@ (* pi 1.5) (getvar "VIEWSIZE")))
  (command "UNDO" "BEGIN")
  (if (ssget "X")
  ...
  (setvar "UCSICON" 0)
  (foreach Block$ BlkList@
    (setq PathSld$ (strcat BlockLibBase$ "\\Custom\\KTO_Sld\\" Block$))
    (command ".INSERT" Block$ "0,0" "" "" "")
    (command ".ZOOM" "E")
    (setq RealBlk& (ssget "L"))
    (if RealBlk&
...
;-------------------------------------------------------------------------------
; Custom_Dirs - Creates PathDef$, PathBlk$ and C:\Custom\KTO_Sld folders for
...
>>>>>>> origin/master
  ...
  (if (not *KTO_Lib*);Check for global *KTO_Lib* variable
    (KTO_Lib_Msg);Installation Instructions
  );if
  ;-----------------------------------------------------------------------------
  ...
  ;-----------------------------------------------------------------------------
  ; Create C:\Custom\KTO_Temp.bat file to create new folders and files.
  ;-----------------------------------------------------------------------------
  (setq FileName% (open (strcat BlockLibBase$ "\\Custom\\KTO_Temp.bat") "w"))
  (write-line ":---- KTO_Temp.bat ----" FileName%)
  (write-line "@ECHO OFF" FileName%)
  (write-line "C:" FileName%)
  (if (and
      (/= PathDef$ (strcat "\"" BlockLibBase$ "\\Custom\\KTO_Lib\\KTO_Lib.def" "\""))
      (/= PathDef$ (strcat "\""  BlockLibBase$ "\\Custom\\KTO_Lib\\Sel_Lib.dat" "\""))
      (/= PathDef$ (strcat "\""  BlockLibBase$ "\\Custom\\KTO_Sld\\KTO_Sld.def" "\""))
    );and
    (progn
  ...
  (setq PathFile$ (strcat "\"" BlockLibBase$ "\\Custom\\KTO_Lib\\KTO_Lib.def" "\""))
  (if (not (findfile PathFile$))
    (progn
      ...
      (setq Echo2$ (strcat "ECHO ; KTO_Lib.def >> " PathFile$))
  ...
  (setq PathFile$ (strcat  "\"" BlockLibBase$ "\\Custom\\KTO_Lib\\Sel_Lib.dat" "\""))
  (if (not (findfile PathFile$))
    (progn
      (vl-mkdir (strcat BlockLibBase$ "\\Custom\\KTO_Lib"))
      ...
      (setq WriteText$ (strcat
        Q$ "Block" Q$ "," Q$ "\\Custom\\KTO_Lib\\KTO_Lib.def" Q$ ","
        Q$ "\\Custom\\KTO_Lib\\KTO_Lib.def" Q$)
      );setq
  ...
  (setq PathFile$ (strcat "\"" BlockLibBase$ "\\Custom\\KTO_Sld\\KTO_Sld.def" "\""))
  (if (not (findfile PathFile$))
    (progn
      (vl-mkdir (strcat BlockLibBase$ "\\Custom\\KTO_Sld"))
      (setq Path3$ (strcat PathFile$ "\n"))
      (setq Echo1$ (strcat "ECHO " Text$ " > " PathFile$))
      (setq Echo2$ (strcat "ECHO ; KTO_Sld.def >> " PathFile$))
      (setq Echo3$ (strcat "ECHO " Text$ ">> " PathFile$))
      (write-line Echo1$ FileName%)
      (write-line Echo2$ FileName%)
      (write-line Echo3$ FileName%)
  ...
  (setq PathFile$ (strcat BlockLibBase$ "\\Temp\\Temp.dcl"))
  (if (not (findfile PathFile$))
    (progn
      (vl-mkdir (strcat BlockLibBase$ "\\Temp"))
  ...
  (if MadeIt
    (progn
      (runapp (strcat "\"" BlockLibBase$ "\\Custom\\KTO_Temp.bat"  "\"" ) nil)
...
;-------------------------------------------------------------------------------
(defun c:Dwg_Blks (/ KTO_Sld_def: BlkList@ Dwg_Blks: Old_error TblList@
  );variables
  (setvar "ATTREQ" 0) (setvar "CMDECHO" 0) (setvar "REGENMODE" 1)
  ;-----------------------------------------------------------------------------
  ; c:Dwg_Blks subfunctions = KTO_Sld_def:, Dwg_Blks:
  ;-----------------------------------------------------------------------------
  ; KTO_Sld_def: - Create KTO_Sld.def file.
  ;-----------------------------------------------------------------------------
  (defun KTO_Sld_def: (BlkList@ / Attribs$ BlockData$ FileName% PathFile$ Q$ Text$
    );variables
    (setq Text$ (strcat ";" (String$ 79 "-")))
    (setq PathFile$ (strcat BlockLibBase$ "\\Custom\\KTO_Sld\\KTO_Sld.def"))
    (setq FileName% (open PathFile$ "w"))
    (write-line Text$ FileName%)
    (write-line "; KTO_Sld.def" FileName%)
    (write-line Text$ FileName%)
    (setq Q$ "\"")
    (foreach Block$ BlkList@
      (if (= (cdr (assoc 70 (tblsearch "BLOCK" Block$))) 2)
...
>>>>>>> origin/master
  ...
  ;-----------------------------------------------------------------------------
  (defun Dwg_Blks: (LibTitle$ DefFile$ / Attribs$ Trim$ Allign$ AttribsList@ BlkList@ Block$
      BlockLen# Option# Cnt# Dcl_Id% EOF Explode$ ExplodeList@ TrimList@ AllignList@ Field# FileName%
    ...
    ;---------------------------------------------------------------------------
    (if (setq PathFile$ (findfile (strcat (strcat BlockLibBase$ "\\Custom\\KTO_Sld\\" DefFile$))))
  ...
  (if BlkList@
    (progn
      ...
      (if BlkList@
        (progn
          (KTO_Sld_def: BlkList@)
          (Dwg_Blks: "Drawing Blocks" "KTO_Sld.def")
        );progn
...
(defun Edit_Lib (LibTitle$ PathDef$ / AttribsList@ BlkList@ BlockLen# Option#
    Cnt# DefFile$ DefPath$ Dcl_Id% EOF ExplodeList@ TrimList@ AllignList@Field# FileName% ImageName$
...
>>>>>>> origin/master
  ...
  ;-----------------------------------------------------------------------------
  ; Load DCL dialog: Edit_Lib in KTO_Lib.dcl
  ;-----------------------------------------------------------------------------
  (princ "\nSelect a block or option: ")(princ)
  (setq Dcl_Id% (load_dialog "KTO_Lib.dcl"))
  (while (> Option# 2)
      ...
      (progn
        ...
        (foreach Item BlkList@
          (setq WriteData@ (strcat
            ...
            Q$(nth Rep# TrimList@)Q$ ","    ;Trim$
            Q$(nth Rep# AllignList@)Q$)     ;Allign$
          );setq
...
; May be used with c:All_KTO_Lib to quickly add blocks to Libraries.
;-------------------------------------------------------------------------------
      ...
      (if (= InsertAll$ "Yes")
        (progn
          ...
          (foreach Block$ FolderDwgs@
            ...
            (if (not (member (strcase Block$) (list (strcase (getvar "DWGNAME")) "KTO_LIB.DWG")))
              (progn
...
;-------------------------------------------------------------------------------
; SaveDwgName - Saves drawing before backing it up as C:\Custom\KTO_Lib.dwg.
;-------------------------------------------------------------------------------
(defun SaveDwgName (/ DwgName$)
  ...
  (if (= (getvar "DWGTITLED") 0)
    (progn
      (setq DwgName$ (strcat BlockLibBase$ "\\Custom\\KTO_Lib.dwg"))
      (if (findfile DwgName$)
  ...
  (if (/= DwgName$ (strcat BlockLibBase$ "\\Custom\\KTO_Lib.dwg"))
    (if (findfile (strcat BlockLibBase$ "\\Custom\\KTO_Lib.dwg"));Backup copy of drawing
      (command ".SAVE" (strcat BlockLibBase$ "\\Custom\\KTO_Lib.dwg") "Y")
      (command ".SAVE" (strcat BlockLibBase$ "\\Custom\\KTO_Lib.dwg"))
    );if
  );if
  (princ)
);defun SaveDwgName
...
>>>>>>> origin/master
              ...
              (if (and NewBlkPath$
                  (= (car (GetPathFile NewBlkPath$)) (strcat BlockLibBase$ "\\Custom\\KTO_Sld\\"))
                );and
                (progn
                  (GetOK DialogTitle$ (strcat
                    BlockLibBase$ "\\Custom\\KTO_Sld folder is used by Drawing Blocks\n"
              ...
              (if (and NewBlkPath$
                  (= (car (GetPathFile NewBlkPath$)) (strcat BlockLibBase$ "\\Custom\\KTO_Lib\\"))
                );and
                (progn
                  (GetOK DialogTitle$ (strcat
                    BlockLibBase$ "\\Custom\\KTO_Lib folder is used and defined\n"
              ...
              (if (and NewSldPath$
                  (= (car (GetPathFile NewSldPath$)) (strcat BlockLibBase$ "\\Custom\\"))
                );and
                (progn
                  (GetOK DialogTitle$ (strcat
                    "No new folder was created inside BlockLibBase\\Custom.\n"
...
>>>>>>> origin/master
              ...
              (if (and NewSldPath$
                ...
                (progn
                  (GetOK DialogTitle$ (strcat
                    "No new folder was created inside BlockLibBase\\BlockLib\\Custom.\n"
                    "Choose or create a new folder for the\n"
                    Title$ ".def file for the " Title$ " Library.") "Filefolder"
                  );GetOK
                  (setq NewSldPath$ nil)
    ...
    ;---------------------------------------------------------------------------
    ; Load DCL dialog: Add_Lib_Defs or Edit_Lib_Defs in KTO_Lib.dcl
    ;---------------------------------------------------------------------------
    (setq Cloice# 99)
    (if (/= SldPath$ "")
  ...
  (defun Sel_Lib_dat: (TitleLib@ SlideLib@ BlockLib@ / FileName% Text$ Q$ Rep#
    ...
    (foreach Item TitleLib@
      (setq WriteData@ (strcat
        ...
        Q$(substr (nth Rep# SlideLib@)(+(strlen BlockLibBase$)1))  Q$ "," ;PathDef$
        Q$(substr (nth Rep# BlockLib@)(+(strlen BlockLibBase$)1))Q$)    ;PathBlk$
      );setq
      (write-line WriteData@ FileName%)
      (setq Rep# (1+ Rep#))
    );foreach
  ...
  ;-----------------------------------------------------------------------------
  ; Load DCL dialog: Sel_Lib in KTO_Lib.dcl
  ;-----------------------------------------------------------------------------
  (setq Cloice# 99)
  (if (and (= (type *KTO_Lib*) 'STR) (member *KTO_Lib* TitleLib@))
    (setq intLib# (- (length TitleLib@) (length (member *KTO_Lib* TitleLib@)))
    ...
    (setq *KTO_Lib* "Block"
          intLib# (- (length TitleLib@) (length (member *KTO_Lib* TitleLib@)))
          nthLibrary$ (itoa intLib#)
    );setq
  );if
  (setq Dcl_Id% (load_dialog "KTO_Lib.dcl"))
...
(defun Slide_Script (PathFilename$ / DefFile$ DwgList@ DwgName$ DwgNames@
  ...
  (foreach DwgName$ DwgNames@
    (setq Text$ (strcat Q$ DwgName$ Q$ "," Q$ "Current layer" Q$ "," Q$
      "User selects" Q$ "," Q$ "User selects" Q$ "," Q$ "0" Q$ "," Q$ "0" Q$ "," Q$ "0" Q$ "," Q$ "0" Q$)
    );setq
    (write-line Text$ FileName%)
  );foreach
  (close FileName%)
  (setq FileName% (open (strcat BlockLibBase$ "\\Temp\\Temp.scr") "w"))
  (setq First t)
...
;-------------------------------------------------------------------------------
(defun User_Lib (LibTitle$ PathDef$ PathBlk$ / Attribs$ Trim$ Allign$ AttribsList@
...
>>>>>>> origin/master
  ...
  (princ "1331321331313131321313231231331313216")
...
(defun c:Add_Dwgs (/ Answer$ BlockLib@ Cnt# DatFile$ Dcl_Id% DefFile$ Dwgname$
  ...
  (setq FileName% (open (strcat BlockLibBase$ "\\Custom\\KTO_Lib.scr") "w"))
  (write-line "FileOpen" FileName%)
  (write-line "Y" FileName%)
  (write-line (strcat Q$ (getvar "DWGPREFIX") (getvar "DWGNAME") Q$) FileName%)
  (close FileName%)
...
;-------------------------------------------------------------------------------
(defun Add_Dwgs (TitleLib$ PathDef$ / Attribs$ Trim$ Allign$ BlkList$ Dcl_Id% DefPath$ Dot$
  ...
  (unload_dialog Dcl_Id%)
  (if (= Option# 0)
    (command "SCRIPT" (strcat BlockLibBase$ "\\Custom\\KTO_Lib.scr"))
  );if
  (if (= Option# 1)
    (progn
      (if (= Scale$ "Full scale")
      ...
      (setq BlkList$ (strcat Q$ Dwgname$ Q$ ","Q$ Layer$ Q$ ","
        Q$ Point$ Q$ "," Q$ Scale$ Q$ "," Q$ Explode$ Q$ "," Q$ Attribs$ Q$ "," Q$ Trim$ Q$ "," Q$ Allign$ Q$)
      );setq
      (setq FileName% (open PathDef$ "a"))
      (write-line BlkList$ FileName%)
      (close FileName%)
...
(defun Select_Dwgs (FolderDwgs@ TitleLib$ Pathname$ / Dcl_Id% DclList@ FileName%
  ...
  (setq FileName% (open (strcat BlockLibBase$ "\\Temp\\Temp.dcl") "w"))
  ...
  (setq Dcl_Id% (load_dialog (strcat BlockLibBase$ "\\Temp\\Temp.dcl")))
...
(defun c:Match (/ Dcl_Id% DoneList@ FileName% Finished First$ FolderSlds@ Free
  ...
  (defun GetSldFolder: (/ NewSldPath$ Num#)
    ...
    (if *SldPath$
      ...
      (setq NewSldPath$ (getfiled
        " Select a slide in a folder for Match Slides"
        (strcat BlockLibBase$ "\\Custom\\") "sld" 2)
      );setq
    );if
    (if (not (findfile (strcat BlockLibBase$ "\\Temp\\Temp.txt")))
      (vl-mkdir (strcat BlockLibBase$ "\\Temp"))
    );if
    (if NewSldPath$
      (progn
  ...
  (defun MatchScores: (Write / FileName% Q QcQ Cnt# Text$)
    ...
    (if Write
      (progn
        (setq FileName% (open (strcat BlockLibBase$ "\\Temp\\Match.dat") "w"))
        (repeat 10
          (setq Text$ (strcat Q
      ...
      (progn
        (setq Scores@ nil)
        (if (findfile (strcat BlockLibBase$ "\\Temp\\Match.dat"))
          (progn
            (setq FileName% (open (strcat BlockLibBase$ "\\Temp\\Match.dat") "r"))
...
;-------------------------------------------------------------------------------
; KTO_Lib_Support - Checks to see if supporting functions are loaded
;-------------------------------------------------------------------------------
(defun KTO_Lib_Support ()
  (vl-load-com)
  (if (not (findfile (strcat BlockLibBase$ "\\Custom\\KTO_Temp.bat")))
    (progn
<<<<<<< HEAD
...
>>>>>>> origin/master
  ...
  (if (not (findfile (strcat BlockLibBase$ "\\Temp\\Temp.scr")))
    (vl-mkdir (strcat BlockLibBase$ "\\Temp"))
  );if
  (if (or (not GetOK)(not EditBox))
    (progn
      ...
      (if (or (not GetOK)(not EditBox))
        (progn
          (alert (strcat "KTO_Lib requires the functions inside of GetIcon.lsp."
